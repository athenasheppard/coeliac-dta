...
*! Author: Roger Harbord, University of Bristol
	...
	/* bivariate meta-analysis of diagnostic accuracy
		...
		Changes since v0.4 :
		2.11 fixed bug in Var(logitSe/p) with "univariate" option - was reporting and plotting SDs instead of variances
		2.1  "symmetric" option (xtmelogit only) to give symmetric SROC
		     (equal variances) by using covariance(exchangeable) option to -xtmelogit-
		2.0  defaults to xtmelogit but with gllamm as option. Added Univariate option (IMPLEMENTED FOR xtmelogit ONLY).
...
program define metandi2, byable(onecall) sortpreserve
version 8.2
...
version 8.2
	syntax varlist(min=4 max=4) [if] [in] ///
	  [, Plot Gllamm force IP(string) NIp(int -1) Univariate SYmmetric ///
...
*/
			...
			summ `tf' if _metandi_d`g' & `touse', meanonly
		...
		foreach g of numlist 1 0 {
			...
			`quietly' xtmelogit _metandi_true _metandi_d`g' if _metandi_d`g' & `touse', ///
			  ...
			  binomial(_metandi_n) intp(`nipuni') ///
		...
		if "`univariate'" != "" {
			matrix define buni = ( b1[1,1], b0[1,1],  b1[1,2], b0[1,2] )
			matrix define Vuni = ( 	V1[1,1], 0,       V1[2,1], 0        \ ///
											0,       V0[1,1], 0,       V0[2,1]  \ ///
											V1[2,1], 0,       V1[2,2], 0        \ ///
											0,       V0[2,1], 0,       V0[2,2]  )
			matrix colnames buni = muA muB lnsA lnsB 	//  -xtmelogit- _b[1,2] estimates are SDs NOT variances
			matrix colnames Vuni = muA muB lnsA lnsB  // Noticed and fixed 2010-12-13
			matrix rownames Vuni = muA muB lnsA lnsB
			ereturn post buni Vuni
			qui nlcom (muA: _b[muA] ) (muB: _b[muB]) ///
				(s2A: exp(2 * _b[lnsA])) (s2B: exp(2 * _b[lnsB]))  // "2 *" converts SDs into variances
			matrix buni = r(b)
			matrix Vuni = r(V)
			matrix b = J(1,5,0)
			matrix V = J(5,5,0)
			matrix b[1,1] = buni
			matrix V[1,1] = Vuni
			matrix colnames b = muA muB s2A s2B sAB
			matrix colnames V = muA muB s2A s2B sAB
			matrix rownames V = muA muB s2A s2B sAB
			qui count if `touse'
			ereturn post b V, obs(`=r(N)') // gets halved later
			local ll = ll0 + ll1
			local hsroc nohsroc
		}
			...
			if "`symmetric'" == "" {
				...
				local atr atr1_1_1_2
				local lns2 lns1_1_2
			}
			else {
				local cov "exchangeable"
				matrix b0 = ( b1[1,1], b0[1,1], ( b1[1,2] + b0[1,2] )/2, tanh(r(rho)))
				local atr atr1_1_1_1
				local lns2 lns1_1_1
			}
				
			/*** fit full bivariate model ***/
			xtmelogit _metandi_true _metandi_d1 _metandi_d0, nocons ///
			  || _metandi_i: _metandi_d1 _metandi_d0, nocons covariance(`cov') ///
			  binomial(_metandi_n) ///
			...
			// c22 v small -> corrlogits=+/-1 & numerical problem if include in nlcom
			local corrterm "tanh([`atr']_b[_cons])"
			if [`atr']_b[_cons] < -4 local corrterm "(-1)"
			if [`atr']_b[_cons] >  4 local corrterm "1"
			...
			qui nlcom ///
			  ...
			  ( s2B: exp(2 * [`lns2']_b[_cons]) ) /// 
			  ( sAB: exp([lns1_1_1]_b[_cons]) * exp([`lns2']_b[_cons])  ///
			  * `corrterm'                        ) ///
			  , post
	...
	if "`univariate'" == "" {
		...
		else {  // symmetric - beta is fixed at 0 not estimated
			qui nlcom ///
			  ( Lambda:     ( _b[s2B] / _b[s2A] )^(.25) * _b[muA] ///
			  + ( _b[s2A] / _b[s2B] )^(.25) * _b[muB]               ) ///
			  ( Theta: .5*( ( _b[s2B] / _b[s2A] )^(.25) * _b[muA] ///
	...
	Display, level(`level') `bivariate' `hsroc' `summarypt' `univariate' `symmetric'
...
program define Display, eclass
	syntax [, Level(real `c(level)') noBivariate noHsroc noSummarypt Univariate SYmmetric ]
...
// di as txt "Condition number = " as res  e(cn)
	...
	if "`bivariate'" == "" {
		...
		if "`univariate'" == "" di in smcl as res "Bivariate" as txt "{col 14}{c |}"
		else di in smcl as res "2 Univariates" as txt "{col 14}{c |}"
	...
	if "`hsroc'" == "" {
		if "`symmetric'" == "" 	RenamePost b_hsroc V_hsroc ///
			"Lambda:_cons Theta:_cons beta:_cons s2alpha:_cons s2theta:_cons"
		else RenamePost b_hsroc V_hsroc ///
			"Lambda:_cons Theta:_cons s2alpha:_cons s2theta:_cons"
		di in smcl as txt "{hline 13}{c +}{hline 64}"
		...
		if "`symmetric'" == "" _diparm beta, label(beta) level(`level')
		else di in smcl as txt %12s "beta" "{col 14}{c |}"  ///
		  as txt "{col 17}" %9s "0 (fixed)" // "{col 36}.{col 66}.{col 78}."
		...
		else _diparm s2alpha s2theta,  /// s2theta is dummy
		...
		else _diparm s2theta s2alpha,  /// s2alpha is dummy
